...
int set_non_blocking_socket (ortp_socket_t sock)
{
        ...
        long b = 1;
        return setsockopt(sock, SOL_SOCKET, SO_NONBLOCK, &b, sizeof(b));
	#else
...
#if	!defined(_WIN32) && !defined(_WIN32_WCE) && !defined(PSP)
...
int psp_mutex_init(ortp_mutex_t *m, void *attr_unused)
{
	*m = sceKernelCreateSema("rtpM", 0, 1, 1, 0);
	return 0;
}

int psp_mutex_lock(ortp_mutex_t *mutex)
{
	sceKernelWaitSema(*mutex, 1, 0);
	return 0;
}

int psp_mutex_unlock(ortp_mutex_t *mutex)
{
	sceKernelSignalSema(*mutex, 1);
	return 0;
}

int psp_mutex_destroy(ortp_mutex_t *mutex)
{
...
int psp_cond_init(ortp_cond_t *cond, void *attr)
{
	static int num=0;
	sprintf(temp, "rtpCond%d\n", num);
	*cond = sceKernelCreateEventFlag(temp, PSP_EVENT_WAITMULTIPLE, 0, 0);
    return 0;
}

int psp_cond_wait(ortp_cond_t* cond, ortp_mutex_t * mut)
{
	if(!cond || !mut)
		return -1;

	if(psp_mutex_unlock(mut) < 0)
		return -1;

	sceKernelWaitEventFlag(*cond, 1, PSP_EVENT_WAITCLEAR, NULL, 0);
...
int psp_cond_signal(ortp_cond_t * cond)
{
	if (!cond)
		return -1;
...
int psp_cond_broadcast(ortp_cond_t * cond)
{
	SceKernelEventFlagInfo info;
	int i;

	if (sceKernelReferEventFlagStatus(*cond, &info) >= 0)
	{
		for(i=0; i<info.numWaitThreads; i++)
		{
			psp_cond_signal(cond);
			sceKernelWaitEventFlag(*cond, 0, 0, NULL, 0);
		}
	}
...
int psp_cond_destroy(ortp_cond_t * cond)
{
...
int psp_thread_create(ortp_thread_t *t, void *attr_unused, void *(*func)(void*), void *arg, int size)
{
	thread_data data;
	data.function = (void*)func;
	data.arg = arg;

	*t = sceKernelCreateThread("rtp", thread_func, 0x18, 0x10000, 0, NULL);

	sceKernelStartThread(*t, sizeof(data), &data);
	return 0;
}

int thread_func(SceSize args, void *argp)
{	
	thread_data *pdata = (thread_data*)argp;
	
	(pdata->function)(pdata->arg);

	return 0;
}

int psp_thread_join(ortp_thread_t thread, void **unused)
{
