/*
   --------------------------------------------------------------------------
   stdioprintf.c                                      NanoC Files (PSPE)
   --------------------------------------------------------------------------
   ...
   ----------------------------------------------------------------------------
   Routines di base per la gestione di printf 
   ----------------------------------------------------------------------------
*/   
...
/*
   -----------------------------------------------------------------------
   Routines di scrittura
   -----------------------------------------------------------------------
*/

void _INTERNAL_WriteFncToFile_(char *zNewText, int nNewChar, void *arg)
{    
...
void _INTERNAL_WriteFncToMemory_ (char *zNewText, int nNewChar, void *arg)
{
...
void _INTERNAL_WriteFncToString_ (char *txt, int amt, void *arg)  
{
  char *head;
  const char *t;
  int a;
...
/*           Inseriti durante il debug delle routine
void ndLL_WriteLetter(char Lettera, unsigned short int Color);
void ndLL_WriteLn (char *str, unsigned short color);
void ndDelay (float t);
...
#define SPACESIZE (sizeof(spaces)-1)
  ...
  float rounder;            /* Used for rounding floating point values */
...
int snprintf(char *str, size_t sz, const char *format, ...)
{
	...
	ret = vxprintf(_INTERNAL_WriteFncToString_, &arg, format, args);
	va_end(args);
...
int vsnprintf(char *buf, size_t n, const char *fmt, va_list ap)
{
  ...
  arg.last = &buf[n-1];
  *buf = 0;
  return vxprintf(_INTERNAL_WriteFncToString_,&arg,fmt,ap);
}
...
char *mprintf(const char *zFormat, ...){
  ...
  sMprintf.zText = zBuf;
  sMprintf.zBase = zBuf;
  vxprintf(_INTERNAL_WriteFncToMemory_,&sMprintf,zFormat,ap);
  va_end(ap);
...
int vasprintf(char **strp, const char *format, va_list ap) 
{
  ...
  sMprintf.nAlloc = sizeof(zBuf);
  sMprintf.zBase = zBuf;
  vxprintf(_INTERNAL_WriteFncToMemory_,&sMprintf,format,ap);
...
int fprintf(FILE *pOut, const char *zFormat, ...)
{
  ...
  va_start(ap,zFormat);
  retc = vxprintf(_INTERNAL_WriteFncToFile_,pOut,zFormat,ap);
  va_end(ap);
...
int vfprintf(FILE *pOut, const char *zFormat, va_list ap)
{
  ...
  else
  {
      if (pOut==stderr)
      {
         return errvprintf (zFormat, ap);
      }
      else
      {
         return vxprintf(_INTERNAL_WriteFncToFile_,pOut,zFormat,ap);
      }
  }
}
