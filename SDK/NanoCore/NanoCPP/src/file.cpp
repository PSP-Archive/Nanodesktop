 ...
 *
 * $Id: file.cpp 584989 2007-10-16 01:04:29Z sebor $
 *
 ***************************************************************************
 *
...
#else
#  define _RWSTD_FILENO(f) brdcpp_fileno (f)
#endif
...
__rw_fopen (const char *fname, int openmode, long prot)
{
    ...
    if (fname) {
        // open the named file
        fd = brdcpp_open (fname, fdmode, prot);
    }
    else {
...
// FIXME -- need to have equivalent of fcntl() on win32.
   ...
   const int m = brdcpp_fcntl (fd, _RWSTD_F_GETFL);
...
__rw_fseek (void *file, int flags, _RWSTD_PTRDIFF_T offset, int origin)
{
    ...
    return brdcpp_seek (fd, offset, origin);
}
...
__rw_fread (void *file, int flags, void *buf, _RWSTD_SIZE_T size)
{
    ...
    return brdcpp_read (fd, buf, size);
}
...
__rw_fwrite (void *file, int flags, const void *buf, _RWSTD_SIZE_T size)
{
    ...
    return brdcpp_write (fd, buf, size);
}
