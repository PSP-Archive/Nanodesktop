                  ...
                  const char *macro_name, const char *fmt, int prec)
{
    if ('-' == *xstr || '0' <= *xstr && '9' >= *xstr) {
        cfgprintf ("#define _RWSTD_%-16s %s \n", macro_name, xstr);
    }
    else {
        cfgprintf ("#define _RWSTD_%-16s ", macro_name);
        cfgprintf (fmt, prec + 2, x);
        cfgprintf ("\n");
    }
    puts ("");
}
...
#define PRINTINT(macro)                                         \
    if (-1 == (macro))                                          \
        cfgprintf ("%s \n");                                    \
        cfgprintf ("#define _RWSTD_%-16s %6d \n", #macro, (macro)) \
...
#  define LDBL_FMT   "L"
    ...
    cfgprintf ("#define _RWSTD_%-16s %6d   /* %s */\n",
...
#  define ERANGE -1
    ...
    if (f < 0.0 || f > 1.0 || !errno)
        cfgprintf ("#define _RWSTD_NO_STRTOF_UFLOW\n");
    else
        cfgprintf ("// #define _RWSTD_NO_STRTOF_UFLOW    // %d%s\n",
    ...
    if (d < 0.0 || d > 1.0 || !errno)
        cfgprintf ("#define _RWSTD_NO_STRTOD_UFLOW\n");
    else
        cfgprintf ("// #define _RWSTD_NO_STRTOD_UFLOW    // %d%s\n",
                errno, ERANGE == errno ? " (ERANGE)" : "");
...
#    endif   // HP-UX, _LONG_DOUBLE
    ...
    if (ld < 0.0 || ld > 1.0 || !errno)
        cfgprintf ("#define _RWSTD_NO_STRTOLD_UFLOW\n");
    else
        cfgprintf ("// #define _RWSTD_NO_STRTOLD_UFLOW   // %d%s\n",
                errno, ERANGE == errno ? " (ERANGE)" : "");
