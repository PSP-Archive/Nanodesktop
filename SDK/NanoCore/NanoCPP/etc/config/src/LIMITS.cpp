...
unsigned compute_char_bits ()
{
    ...
    cfgprintf ("#define _RWSTD_CHAR_BIT    %2u\n", bits);
...
int LIMITS_main ()
{
    ...
    cfgprintf ("#define _RWSTD_BOOL_SIZE   %2u /* sizeof (bool) */\n",
            SIZEOF (bool));
#endif   // _RWSTD_NO_BOOL

    cfgprintf ("#define _RWSTD_CHAR_SIZE   %2u /* sizeof (char) */\n",
            SIZEOF (char));
    cfgprintf ("#define _RWSTD_SHRT_SIZE   %2u /* sizeof (short) */\n",
            SIZEOF (short));
    cfgprintf ("#define _RWSTD_INT_SIZE    %2u /* sizeof (int) */\n",
            SIZEOF (int));
    cfgprintf ("#define _RWSTD_LONG_SIZE   %2u /* sizeof (long) */\n",
            SIZEOF (long));

    cfgprintf ("#define _RWSTD_FLT_SIZE    %2u /* sizeof (float) */\n",
            SIZEOF (float));
    cfgprintf ("#define _RWSTD_DBL_SIZE    %2u /* sizeof (double) */\n",
            SIZEOF (double));

#ifndef _RWSTD_NO_LONG_DOUBLE
    cfgprintf ("#define _RWSTD_LDBL_SIZE   %2u /* sizeof (long double) */\n",
            SIZEOF (long double));
#endif   // _RWSTD_NO_LONG_DOUBLE

    // compute the sizes of data and function pointers
    cfgprintf ("#define _RWSTD_PTR_SIZE    %2u /* sizeof (void*) */\n",
            SIZEOF (void*));

    typedef void (*fun_ptr_t)();
    cfgprintf ("#define _RWSTD_FUNPTR_SIZE %2u /* sizeof (void(*)()) */\n",
            SIZEOF (fun_ptr_t));

    typedef void (EmptyStruct::*memfun_ptr_t)();
    cfgprintf ("#define _RWSTD_MEMPTR_SIZE %2u "
            "/* sizeof (void (struct::*)()) */\n",
            SIZEOF (memfun_ptr_t));
    ...
    cfgprintf ("#define _RWSTD_BOOL_MIN   !!0\n");
    cfgprintf ("#define _RWSTD_BOOL_MAX    !0\n");
    ...
    cfgprintf ("#define _RWSTD_NO_TWOS_COMPLEMENT\n");
...
#    define LLong long long
    ...
    cfgprintf ("#define _RWSTD_LLONG_SIZE  %2u\n", SIZEOF (LLong));
...
#    endif   // _MSC_VER
    ...
    cfgprintf ("#define _RWSTD_WCHAR_SIZE  %2u /* sizeof (wchar_t) */\n",
            SIZEOF (wchar_t));
    ...
    cfgprintf ("#define _RWSTD_MB_LEN_MAX    %d   /* libc value */\n",
            MB_LEN_MAX);

#else   // if !defined (MB_LEN_MAX)

#  if defined (_AIX)
    cfgprintf ("#define _RWSTD_MB_LEN_MAX %d   /* known AIX libc value */\n", 4);
#  elif defined (__hpux)
    cfgprintf ("#define _RWSTD_MB_LEN_MAX %d   /* known HP-UX libc value */\n", 4);
#  elif defined (__sgi) || defined (sgi)
    cfgprintf ("#define _RWSTD_MB_LEN_MAX %d   /* known IRIX libc value */\n", 5);
#  elif defined (__GLIBC__) && __GLIBC_MINOR__ <= 1
    cfgprintf ("#define _RWSTD_MB_LEN_MAX %d   /* known glibc 2.1 value */\n", 6);
#  elif defined (__GLIBC__) && __GLIBC_MINOR__ >= 2
    cfgprintf ("#define _RWSTD_MB_LEN_MAX %d   /* known glibc 2.2 value */\n", 16);
#  elif defined (__sun__) || defined (__sun) || defined (__sun)
    cfgprintf ("#define _RWSTD_MB_LEN_MAX %d   /* known SunOS libc value */\n", 5);
#  elif defined (_WIN32)
    cfgprintf ("#define _RWSTD_MB_LEN_MAX %d   /* known WIN32 libc value */\n", 5);
#  else
    cfgprintf ("#define _RWSTD_MB_LEN_MAX %d   /* guess */\n", 8);
#  endif
...
#define PRINT_SPECIFIC(width, least, type)                              \
    do {                                                                \
        ...
        if (least_str && *least_str)                                    \
            cfgprintf ("#define _RWSTD_INT_LEAST%d_T  %s  _RWSTD_%s_T\n"   \
                    "#define _RWSTD_UINT_LEAST%d_T %s  _RWSTD_U%s_T\n", \
                    width, width < 10 ? " " : "", type,                 \
                    width, width < 10 ? " " : "", type);                \
        else                                                            \
            cfgprintf ("#define _RWSTD_INT%d_T %s          %s%s\n"         \
