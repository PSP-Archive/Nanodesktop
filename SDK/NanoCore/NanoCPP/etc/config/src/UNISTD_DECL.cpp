...
#define MACRO(name)   { name, #name }
        //MACRO (STDIN_FILENO),           // Corretto per compensare il particolare comportamento
        //MACRO (STDOUT_FILENO),          // degli stream di nd
        //MACRO (STDERR_FILENO),
    ...
    // Patch per Nanodesktop
    cfgprintf ("#define _RWSTD_STDIN_FILENO    (int)(&(StreamsArray [0])+1)   \n");
    cfgprintf ("#define _RWSTD_STDOUT_FILENO   (int)(&(StreamsArray [1])+1)   \n");
    cfgprintf ("#define _RWSTD_STDERR_FILENO   (int)(&(StreamsArray [2])+1)   \n");

    for (unsigned i = 0; i != sizeof macros / sizeof *macros; ++i)
        cfgprintf ("#define _RWSTD_%-20s %#8x   // %s\n",
...
int UNISTD_DECL_main ()
{
    ...
    off_t off = 0;
    cfgprintf ("#define _RWSTD_OFF_T %s   // off_t\n", off_t_name (off));

    ssize_t s = 0;
    cfgprintf ("#define _RWSTD_SSIZE_T %s   // ssize_t\n", ssize_t_name (s));
