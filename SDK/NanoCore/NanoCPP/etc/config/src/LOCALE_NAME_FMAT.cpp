...
int print_lc_constants ()
{
    ...
    for (unsigned i = 0; *lc_vars [i].name; ++i) {
        ...
        cfgprintf("#define _RWSTD_%-20s %2d\n",
    ...
    cfgprintf("#define %-27s _RWSTD_%s\n",
                      ...
                      char       &loc_name_cat_eq)
{
    ...
    for (i = 0; lc_vars [i].cat != LC_ALL; ++i) {
        ...
        if (-1 < inx)
            cfgprintf("#define _RWSTD_CAT_%d(pfx) "
                    "{ %d, \"%s\", pfx::_C_%s }%s\n",
                              ...
                              char loc_name_cat_eq)
{
    ...
    if (guess) {
      ...
      && defined (__svr4__)
...
#elif (defined (PSPE_PLATFORM) || defined (PSP_PLATFORM))
        ...
        loc_name_cat_sep     = '/';
        loc_name_cat_eq      = '\0';
        os_name              = "Nanodesktop PSP";

#else

        cfgprintf("// no locales found, unknown system\n");
    ...
    if (os_name)
        cfgprintf("// no locales found, using %s format\n", os_name);

    if (setlocale_environ)
        cfgprintf("// #define _RWSTD_NO_SETLOCALE_ENVIRONMENT\n");
    else
        cfgprintf("#define _RWSTD_NO_SETLOCALE_ENVIRONMENT\n");

    if (loc_name_use_cat)
        cfgprintf("// #define _RWSTD_NO_CAT_NAMES\n");
    else
        cfgprintf("#define _RWSTD_NO_CAT_NAMES\n");

    if (loc_name_cat_sep)
        cfgprintf("#define _RWSTD_CAT_SEP \"%c\"\n", loc_name_cat_sep);
    else
        cfgprintf("#define _RWSTD_NO_CAT_SEP\n");

    if (loc_name_cat_eq)
        cfgprintf("#define _RWSTD_CAT_EQ \"%c\"\n", loc_name_cat_eq);
    else
        cfgprintf("#define _RWSTD_NO_CAT_EQ\n");

    if (loc_name_prepend_sep)
        cfgprintf("// #define _RWSTD_NO_INITIAL_CAT_SEP\n");
    else
        cfgprintf("#define _RWSTD_NO_INITIAL_CAT_SEP\n");

    if (loc_name_condense)
        cfgprintf("// #define _RWSTD_NO_CONDENSED_NAME\n");
    else
        cfgprintf("#define _RWSTD_NO_CONDENSED_NAME\n");
