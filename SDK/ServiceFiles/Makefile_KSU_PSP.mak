...
all-before:
CFLAGS   := $(CFLAGS)    -D"ND_KERNELSERVICES_TO_USER_MAIN" -D"ND_PSP_KERNEL_MODE_APPLICATION"
CXXFLAGS := $(CXXFLAGS)  -D"ND_KERNELSERVICES_TO_USER_MAIN" -D"ND_PSP_KERNEL_MODE_APPLICATION"

all-after:
	$(LD) -v $(XOBJ1) $(XOBJ2) $(XOBJ3) -L$(LIBCFWDEV) --start-group $(OBJ) $(LIBDEV) --end-group -lndSeed_KSU_PSP $(LIBSDK) $(LIBHENSDK) $(XOBJ4) $(XOBJ5) -o eboot.elf -O -s 
	psp-fixup-imports eboot.elf
	mksfo $(BIN) PARAM.SFO 
	psp-strip eboot.elf -o eboot_strip.elf
	pack-pbp EBOOT.PBP PARAM.SFO "$(NDENV_PATH)/PSP/SDK/ServiceFiles/Nanodesktop_PSPKSU_Cover.png" NULL NULL NULL NULL eboot_strip.elf NULL
	rm eboot.elf
	cp eboot_strip.elf eboot.elf
	rm eboot_strip.elf
	rm *.o
	
	xmkdir -p ./pbp_for_psp_ksu
		
	mv eboot.elf ./pbp_for_psp_ksu/EBOOT.ELF
	mv eboot.pbp ./pbp_for_psp_ksu/EBOOT.PBP
