/*  
    ...
    ---------------------------------------------------------------------------
    API PER LA GESTIONE DEL SISTEMA INFRAROSSO (PSPE)
    ---------------------------------------------------------------------------
...
/*
    ...
    seconda della piattaforma: sotto PSPE viene ignorato. 
     ...
     if (ndIrChannelsArray [IrHandleChannel].IsEnabled)
     {
          switch (Mode)
          {
                 case ND_IR_IRDA_MODE:
                 {
                      ...
                      // EMULAZIONE: Non ci sono problemi nell'aprire il device e viene
                      // fornito un HwHandle pari a 0.
