/*  
    ...
    ---------------------------------------------------------------------------
    ENGINE WINDOWS RENDER (PSP KSU MODE)
    ---------------------------------------------------------------------------
...
inline void INTERNAL_DrawMinWindowsBar ()
{
    ...
    if (     (MinWindowsBarIsChanged) ||  
                 ((!ND_HWMODE_DisableUsbService) && (ndUsb_CommArea.UsbStateIsChanged)) ||
                      ((!ND_HWMODE_DisableNetworkService) && (ndNetworkStateIsChanged))       )              
    {
                    ...
                    INTERNAL_ManagerUsbIcon (NotActiveFBPage);        // Aggiorna l'icona USB
                    INTERNAL_ManagerNetworkIcon (NotActiveFBPage);    // Aggiorna l'icona di rete 
...
static inline void INTERNAL_SpecialKernelSleepThread (int ThreadID)
{
...
*/ 
{
   ...
   {      
       ...
       if (IHaveFound)
       {
          ...
          INTERNAL_SpecialKernelSleepThread (ThreadID);
       }
   }
   else                     
   {
