/*  
    ...
    ---------------------------------------------------------------------------
    FUNZIONI HARDWARE DEPENDENT PER GESTIONE GRAFICA (PSP KSU mode)
    ---------------------------------------------------------------------------
*/
  ...
  //  è sempre l'indirizzo virtuale, e non quello reale.
{
    ...
    sceDisplaySetMode     (0, MaxScreenX, MaxScreenY);
    sceDisplaySetFrameBuf (XAddrVideoMemory, VMaxScreenX, 1, 1);
...

{
         ...
         sceDisplayWaitVblankStart();                                                                  // Attende il segnale della routine di rinfresco
                                                                                                       // (impedisce il tearing dell'immagine)

         sceDisplaySetFrameBuf ((XAddrVideoMemory+ActiveFBPage*PageSize), VMaxScreenX, 1, 1);           // Questa routine provoca la commutazione dello schermo
         ...
         LastLineWritten=0;
         WRCurrentLineAddr=WRCurrentPageAddr;                                                           // Occorre aggiornare anche la variabile che tiene conto                                                                                                        // dell'indirizzo di riga                                                                                               
}
