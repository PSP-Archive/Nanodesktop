...
*/
{
    ...
    {      
          if (ndNetInit_IsFailure!=1)
          {              
                   // Su firmware 1.5, modalità KSU, questa routine provvede a 
                   // reinizializzare quei drivers di rete che possono essere 
                   // reinizializzati solo in kernel mode.
                   
                   // Su CF, il processo di inizializzazione deve essere eseguito
                   // all'avvio, nella routine ndHAL_InitLowLevelNetwork, prima
                   // del caricamento del kernel extender. 
                   
                   // Questo significa che ndHAL_WLAN_InitNetworkLayer diventa
                   // praticamente inutile. Viene comunque mantenuta per
                   // ragioni di compatibilità, semplicemente riportando all'
                   // esterno il codice di errore rilevato dalla routine di avvio 
                   
                   if (!ndNetLayer_HasBeenInit)
                   {
                         if (ndNetInit_IsFailure==0)        // Non ci sono stati errori in inizializzazione
                         {                                  // nella fase di avvio
                              ndNetLayer_HasBeenInit = 1;
                              return 0;                      
                         }
                         else                               // Riporta in output il codice di errore principale 
                         {                                  // che era stato rilevato in avvio
                              ndNetLayer_HasBeenInit = 0;
                              return ndNetInit_Failure_MainErrCode; 
                         }
                   }
