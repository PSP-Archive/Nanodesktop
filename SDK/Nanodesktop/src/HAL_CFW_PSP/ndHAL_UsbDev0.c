...
*/
{
	...
	if (retVal != 0) 
    {
        ndEyesvr_IsFailure                 = 1;
	    ndEyesvrFailure_MainErrCode        = ERR_USB_FAILED_INIT_HOSTFS_DRIVER;
        ndEyesvrFailure_SecErrCode         = retVal;
	...
	{
        ndEyesvr_IsFailure               = 1;
        ndEyesvrFailure_MainErrCode      = ERR_USB_FAILED_ACTIVATE_USB_PID;
        ndEyesvrFailure_SecErrCode       = retVal;     
	...
	sceKernelDelayThread (2000000);    // Ritardo di due secondi per consentire al sistema di stabilizzarsi
	return 0;
}

void INTERNAL_EYESVR_UnLoadDeviceDrivers (void)
{
    if (ndEyesvrLoadDepth>=2)  sceUsbDeactivate (HOSTFSDRIVER_PID);
    if (ndEyesvrLoadDepth>=1)  sceUsbStop (HOSTFSDRIVER_NAME, 0, 0); 
