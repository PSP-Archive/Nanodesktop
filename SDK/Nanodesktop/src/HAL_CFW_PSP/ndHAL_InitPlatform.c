/*  
    ...
    ---------------------------------------------------------------------------
    FUNZIONI DI INIZIALIZZAZIONE DELLA PIATTAFORMA (PSP CFW mode)
    ---------------------------------------------------------------------------
*/
...
void ndHAL_InitPlatform_StepA (void)
{
   ...
   ndHAL_InitLowLevelNetwork (); // Carica i drivers a basso livello per la gestione della rete
                                 // Il caricamento deve essere fatto **prima** del kernel extender
                             
   ndHAL_LoadKernelExtender ();  // Caricamento del kernel extender 
   ...
   ndHAL_InitJpegDecoder();       // Inizializza il decoder JPEG hardware
...
{
   // Avvia i servizi specifici per la modalità CFW
