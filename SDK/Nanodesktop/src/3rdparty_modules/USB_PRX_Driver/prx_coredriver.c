/* ------------------------------------------------------------------------
   USB Driver PRX for Nanodesktop PSP
   Version 1.0
   ------------------------------------------------------------------------
...
PSP_MODULE_INFO(ND000_MODULE_NAME, PSP_MODULE_KERNEL, 1, 0);
...
/* Read/Write buffer */
unsigned char tx_buf [256*1024] __attribute__((aligned(64)));
...
int start_func(int size, void *p)
{
        ...
        g_thid = sceKernelCreateThread("USBThread", usb_thread, 2, 0x16000, 0, NULL);
        if(g_thid < 0)
        {
