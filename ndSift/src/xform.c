...
int calc_min_inliers( int, int, xdouble, xdouble );
...
CvMat* ransac_xform( struct feature* features, int n, int mtype,
		     ransac_xform_fn xform_fn, int m, xdouble p_badxform,
		     ransac_err_fn err_fn, xdouble err_tol,
		     struct feature*** inliers, int* n_in )
{
  ...
  CvMat* M = NULL;
  gsl_rng* rng;
  xdouble p, in_frac = RANSAC_INLIER_FRAC_EST;
...
*/
xdouble homog_xfer_err( CvPoint2D64f pt, CvPoint2D64f mpt, CvMat* H )
{
...
{
  CvMat XY, UV;
  xdouble xy[3] = { pt.x, pt.y, 1.0 }, uv[3] = { 0 };
  CvPoint2D64f rslt;
...
*/
int calc_min_inliers( int n, int m, xdouble p_badsupp, xdouble p_badxform )
{
  xdouble pi, sum;
  int i, j;
...
int find_consensus( struct feature** features, int n, int mtype,
		    CvMat* M, ransac_err_fn err_fn, xdouble err_tol,
