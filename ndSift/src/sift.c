...
#define sift_sqrt ndHAL_XFPU_sqrtf

/************************* Local Function Prototypes *************************/

IplImage* create_init_img( IplImage*, int, xdouble );
IplImage* convert_to_gray32( IplImage* );
IplImage*** build_gauss_pyr( IplImage*, int, int, xdouble );
IplImage* downsample( IplImage* );
IplImage*** build_dog_pyr( IplImage***, int, int );
CvSeq* scale_space_extrema( IplImage***, int, int, xdouble, int, CvMemStorage*);
int is_extremum( IplImage***, int, int, int, int );
struct feature* interp_extremum( IplImage***, int, int, int, int, int, xdouble);
void interp_step( IplImage***, int, int, int, int, xdouble*, xdouble*, xdouble* );
CvMat* deriv_3D( IplImage***, int, int, int, int );
CvMat* hessian_3D( IplImage***, int, int, int, int );
xdouble interp_contr( IplImage***, int, int, int, int, xdouble, xdouble, xdouble );
struct feature* new_feature( void );
int is_too_edge_like( IplImage*, int, int, int );
void calc_feature_scales( CvSeq*, xdouble, int );
void adjust_for_img_dbl( CvSeq* );
void calc_feature_oris( CvSeq*, IplImage*** );
xdouble* ori_hist( IplImage*, int, int, int, int, xdouble );
int calc_grad_mag_ori( IplImage*, int, int, xdouble*, xdouble* );
void smooth_ori_hist( xdouble*, int );
xdouble dominant_ori( xdouble*, int );
void add_good_ori_features( CvSeq*, xdouble*, int, xdouble, struct feature* );
struct feature* clone_feature( struct feature* );
void compute_descriptors( CvSeq*, IplImage***, int, int );
xdouble*** descr_hist( IplImage*, int, int, xdouble, xdouble, int, int );
void interp_hist_entry( xdouble***, xdouble, xdouble, xdouble, xdouble, int, int);
void hist_to_descr( xdouble***, int, int, struct feature* );
void normalize_descr( struct feature* );
int feature_cmp( void*, void*, void* );
void release_descr_hist( xdouble****, int );
...
int _sift_features( IplImage* img, struct feature** feat, int intvls,
		    xdouble sigma, xdouble contr_thr, int curv_thr,
		    int img_dbl, int descr_width, int descr_hist_bins )
{
  ...
  octvs = ndHAL_XFPU_logf ( MIN( init_img->width, init_img->height ) ) / ndHAL_XFPU_logf (2) - 2;
  ...
  if( img_dbl ) adjust_for_img_dbl( features );
...
*/
IplImage* create_init_img( IplImage* img, int img_dbl, xdouble sigma )
{
  IplImage* gray, * dbl;
  xdouble sig_diff;
  ...
  if( img_dbl )
    {
      sig_diff = sift_sqrt( sigma * sigma - SIFT_INIT_SIGMA * SIFT_INIT_SIGMA * 4 );
      dbl = cvCreateImage( cvSize( img->width*2, img->height*2 ),
			   IPL_DEPTH_32F, 1 );
      cvResize( gray, dbl, CV_INTER_CUBIC );
      //cvSmooth( dbl, dbl, CV_GAUSSIAN, 0, 0, sig_diff, sig_diff ); MODIFIED FOR ND
      cvSmooth( dbl, dbl, CV_GAUSSIAN, 0, 0, sig_diff);
      cvReleaseImage( &gray );
      return dbl;
    }
  else
    {
      sig_diff = sift_sqrt( sigma * sigma - SIFT_INIT_SIGMA * SIFT_INIT_SIGMA );
      //cvSmooth( gray, gray, CV_GAUSSIAN, 0, 0, sig_diff, sig_diff ); MODIFIED FOR ND
      cvSmooth( gray, gray, CV_GAUSSIAN, 0, 0, sig_diff);
      return gray;
    }
}
...
IplImage*** build_gauss_pyr( IplImage* base, int octvs,
			     int intvls, xdouble sigma )
{
  IplImage*** gauss_pyr;
  const int _intvls = intvls;
  xdouble sig[_intvls+3], sig_total, sig_prev, k;
  int i, o;
  ...
  for( i = 1; i < intvls + 3; i++ )
    {
      ...
      sig[i] = sift_sqrt( sig_total * sig_total - sig_prev * sig_prev );
    }
	...
	/* blur the current octave's last image to create the next one */
	else
	  {
	    gauss_pyr[o][i] = cvCreateImage( cvGetSize(gauss_pyr[o][i-1]),
					     IPL_DEPTH_32F, 1 );
	    /*cvSmooth( gauss_pyr[o][i-1], gauss_pyr[o][i], MODIFIED FOR ND
		      CV_GAUSSIAN, 0, 0, sig[i], sig[i] );
		      */
   	    cvSmooth( gauss_pyr[o][i-1], gauss_pyr[o][i],
		      CV_GAUSSIAN, 0, 0, sig[i]);
	  }
      }
...
CvSeq* scale_space_extrema( IplImage*** dog_pyr, int octvs, int intvls,
			    xdouble contr_thr, int curv_thr,
			    CvMemStorage* storage )
{
  CvSeq* features;
  xdouble prelim_contr_thr = 0.5 * contr_thr / intvls;
  struct feature* feat;
...
{
  xdouble val = pixval32f( dog_pyr[octv][intvl], r, c );
  int i, j, k;
...
struct feature* interp_extremum( IplImage*** dog_pyr, int octv, int intvl,
				 int r, int c, int intvls, xdouble contr_thr )
{
  struct feature* feat;
  struct detection_data* ddata;
  xdouble xi, xr, xc, contr;
  int i = 0;
  ...
  feat = new_feature();
  ddata = feat_detection_data( feat );
  feat->img_pt.x = feat->x = ( c + xc ) * powf( 2.0, octv );
  feat->img_pt.y = feat->y = ( r + xr ) * powf( 2.0, octv );
  ddata->r = r;
  ddata->c = c;
  ddata->octv = octv;
  ddata->intvl = intvl;
...
void interp_step( IplImage*** dog_pyr, int octv, int intvl, int r, int c,
		  xdouble* xi, xdouble* xr, xdouble* xc )
{
  CvMat* dD, * H, * H_inv, X;
  xdouble x[3] = { 0 };
...
{
  CvMat* H;
  xdouble v, dxx, dyy, dss, dxy, dxs, dys;
...
*/
xdouble interp_contr( IplImage*** dog_pyr, int octv, int intvl, int r,
		     int c, xdouble xi, xdouble xr, xdouble xc )
{
  CvMat* dD, X, T;
  xdouble t[1], x[3] = { xc, xr, xi };
...
*/
void calc_feature_scales( CvSeq* features, xdouble sigma, int intvls )
{
  struct feature* feat;
  struct detection_data* ddata;
  xdouble intvl;
  ...
  for( i = 0; i < n; i++ )
    {
      ...
      feat->scl = sigma * powf ( 2.0, ddata->octv + intvl / intvls );
      ddata->scl_octv = sigma * powf ( 2.0, intvl / intvls );
    }
}
...
*/
xdouble* ori_hist( IplImage* img, int r, int c, int n, int rad, xdouble sigma)
{
  xdouble* hist;
  xdouble mag, ori, w, exp_denom, PI2 = CV_PI * 2.0;
  int bin, i, j;

  hist = calloc( n, sizeof( xdouble ) );
...
*/
int calc_grad_mag_ori( IplImage* img, int r, int c, xdouble* mag, xdouble* ori )
{
  xdouble dx, dy;
...
*/
void smooth_ori_hist( xdouble* hist, int n )
{
  xdouble prev, tmp, h0 = hist[0];
  int i;
...
*/
xdouble dominant_ori( xdouble* hist, int n )
{
  xdouble omax;
  int maxbin, i;
...
*/
void add_good_ori_features( CvSeq* features, xdouble* hist, int n,
			    xdouble mag_thr, struct feature* feat )
{
  struct feature* new_feat;
  xdouble bin, PI2 = CV_PI * 2.0;
  int l, r, i;
...
*/
xdouble*** descr_hist( IplImage* img, int r, int c, xdouble ori,
		      xdouble scl, int d, int n )
{
  xdouble*** hist;
  xdouble cos_t, sin_t, hist_width, exp_denom, r_rot, c_rot, grad_mag,
    grad_ori, w, rbin, cbin, obin, bins_per_rad, PI2 = 2.0 * CV_PI;
  int radius, i, j;

  hist = calloc( d, sizeof( xdouble** ) );
  
  for( i = 0; i < d; i++ )
  {
      hist[i] = calloc( d, sizeof( xdouble* ) );
      
      for( j = 0; j < d; j++ )
	       hist[i][j] = calloc( n, sizeof( xdouble ) );
  }
  
  cos_t = ndHAL_XFPU_cosf( ori );
  sin_t = ndHAL_XFPU_sinf( ori );
  bins_per_rad = n / PI2;
  exp_denom = d * d * 0.5;
  hist_width = SIFT_DESCR_SCL_FCTR * scl;
  radius = hist_width * sift_sqrt(2) * ( d + 1.0 ) * 0.5 + 0.5;
...
/*
  ...
  @param n number of bins per orientation histogram
*/
void interp_hist_entry( xdouble*** hist, xdouble rbin, xdouble cbin,
			xdouble obin, xdouble mag, int d, int n )
{
  xdouble d_r, d_c, d_o, v_r, v_c, v_o;
...
*/
void hist_to_descr( xdouble*** hist, int d, int n, struct feature* feat )
{
...
{
  xdouble cur, len_inv, len_sq = 0.0;
  int i, d = feat->d;
...
void release_descr_hist( xdouble**** hist, int d )
{
  register int i, j;
