...
//#include <gdk/gdk.h> MODIFIED FOR ND
//#include <gtk/gtk.h>
...
*/
xdouble dist_sq_2D( CvPoint2D64f p1, CvPoint2D64f p2 )
{
  xdouble x_diff = p1.x - p2.x;
  xdouble y_diff = p1.y - p2.y;
...
void display_big_img( IplImage* img, char* title )
{
  ...
  xdouble img_aspect, scr_aspect, scale;
  ...
  if( img->width >= 0.90 * scr_width  ||  img->height >= 0.90 * scr_height )
    {
      img_aspect = (xdouble)(img->width) / img->height;
      scr_aspect = (xdouble)(scr_width) / scr_height;
...
{
  if( ! cvGetNdWindowHandler(win_name) )
    return 1;
  return 0;
}
