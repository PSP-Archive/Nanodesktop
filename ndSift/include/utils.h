...
#ifndef _SIFT_DOUBLE_DEFINED
        #define _SIFT_DOUBLE_DEFINED
        #define xdouble float    // For nd compatibility
#endif
...
/**
   ...
   @return Returns the value of the pixel at (\a r, \a c) in \a img
*/
static inline xdouble pixval64f( IplImage* img, int r, int c )
{
  return (xdouble)( ( (xdouble*)(img->imageData + img->widthStep*r) )[c] );
}
...
/**
   ...
   @param val pixel value
*/
static inline void setpix64f( IplImage* img, int r, int c, xdouble val )
{
  ( (xdouble*)(img->imageData + img->widthStep*r) )[c] = val;
}
...
/**
   ...
   @param array pointer to an array whose size is to be xdoubled
...
*/
extern int array_xdouble( void** array, int n, int size );
...
*/
extern xdouble dist_sq_2D( CvPoint2D64f p1, CvPoint2D64f p2 );
