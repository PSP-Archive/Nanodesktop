...
ILboolean iSavePnmInternal()
{
    ...
    // PATCH: Support for PBM (by Filippo Battaglia).
    // When you select .pbm format, the system choose Type=IL_PBM_ASCII, but
    // in original code, the following check (if (iGetHint (IL_COMPRESSION_HINT)...
    // add 3 to Type and changes it to IL_PBM_BINARY. But the author of
    // ndDevIL doesn't want to support IL_PBM_BINARY (why?) so the file will be
    // not saved. The result is that original DevIL doesn't support .PBM
    // save. The solution is to limit the check only in the case that 
    // Type is different from IL_PBM_ASCII. 
    ...
    else  // Attempt to use PBM as PBM ASCII.
    {
          Binary = IL_FALSE;
    } 
	...
	switch (Type)
	{
		...
		case IL_PGM_ASCII:
            ...
            // PATCH: Support for PGM (by Filippo Battaglia).
            // There is evidently a bug here, because the algorithm needs to use
            // a gray-level image, that can be obtained using IL_LUMINANCE criteria
		...
		for (j = 0; j < Bpp; j++) {
			if (Binary) {
				...
				if (Type == IL_PBM_ASCII) {
                    ...
                    // PATCH (by Filippo Battaglia)
                    // The order of 1 and 0 is wrong
                    
                    LinePos += ilprintf("%d ", TempData[i] > 127 ? 0 : 1);
                }
				else {
