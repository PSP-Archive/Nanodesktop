...
int
_TIFFNoSeek(TIFF* tif, utiff_int32 off)
{
	(void) off;
	TIFFErrorExt(tif->tif_clientdata, tif->tif_name,
...
{
	const TIFFCodec *c = TIFFFindCODEC((utiff_int16) scheme);
...
const TIFFCodec*
TIFFFindCODEC(utiff_int16 scheme)
{
	const TIFFCodec* c;
	codec_t* cd;
...
TIFFCodec*
TIFFRegisterCODEC(utiff_int16 scheme, const char* name, TIFFInitMethod init)
{
	codec_t* cd = (codec_t*)
