...
_TIFFmemset(tdata_t p, int v, tsize_t c)
{
	...
	while (c > 0) {
		tsize_t chunk = 0x10000 - ((utiff_int32) pp & 0xffff);/* What's left in segment */
...
_TIFFmemcmp(const tdata_t d, const tdata_t s, tsize_t c)
{
	...
	while (c > 0) {
		chunks = 0x10000 - ((utiff_int32) ss & 0xffff);	/* What's left in segment */
		chunkd = 0x10000 - ((utiff_int32) dd & 0xffff);	/* What's left in segment */
