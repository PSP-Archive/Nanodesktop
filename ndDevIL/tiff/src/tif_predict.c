...
{
	tsize_t stride = PredictorState(tif)->stride;
	utiff_int16* wp = (utiff_int16*) cp0;
	tsize_t wc = cc / 2;
...
{
	tsize_t stride = PredictorState(tif)->stride;
	utiff_int32 bps = tif->tif_dir.td_bitspersample / 8;
	tsize_t wc = cc / bps;
	tsize_t count = cc;
	uint8 *cp = (uint8 *) cp0;
	...
	if (cc > stride) {
		...
		if (stride == 3) {
			...
			do {
				...
				cp += 4;
			} while ((tiff_int32)(cc -= 4) > 0);
		} else {
			cp += cc - 1;
			do {
				REPEAT4(stride, cp[stride] -= cp[0]; cp--)
			} while ((tiff_int32)(cc -= stride) > 0);
		}
	}
}
...
PredictorVSetField(TIFF* tif, ttag_t tag, va_list ap)
{
	...
	case TIFFTAG_PREDICTOR:
		sp->predictor = (utiff_int16) va_arg(ap, int);
...
PredictorVGetField(TIFF* tif, ttag_t tag, va_list ap)
{
	...
	switch (tag) {
	case TIFFTAG_PREDICTOR:
		*va_arg(ap, utiff_int16*) = sp->predictor;
		break;
	default:
