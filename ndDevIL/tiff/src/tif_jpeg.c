...
typedef	struct {
	...
	utiff_int16		photometric;	/* copy of PhotometricInterpretation */
	utiff_int16		h_sampling;	/* luminance sampling factors */
	utiff_int16		v_sampling;
	...
	utiff_int32		jpegtables_length; /* number of bytes in same */
...
JPEGPreDecode(TIFF* tif, tsample_t s)
{
	...
	utiff_int32 segment_width, segment_height;
		...
		if (sp->cinfo.d.comp_info[0].h_samp_factor != sp->h_sampling ||
		    sp->cinfo.d.comp_info[0].v_samp_factor != sp->v_sampling) {
			    ...
			    if (!_TIFFFindFieldInfo(tif, 33918, TIFF_ANY)) {
				    ...
				    sp->h_sampling = (utiff_int16)
					sp->cinfo.d.comp_info[0].h_samp_factor;
				    sp->v_sampling = (utiff_int16)
					sp->cinfo.d.comp_info[0].v_samp_factor;
			    }
		}
...
JPEGFixupTestSubsampling( TIFF * tif )
{
    ...
    TIFFSetField( tif, TIFFTAG_YCBCRSUBSAMPLING, 
                  (utiff_int16) sp->h_sampling, (utiff_int16) sp->v_sampling );
...
static utiff_int32
JPEGDefaultStripSize(TIFF* tif, utiff_int32 s)
{
	JPEGState* sp = JState(tif);
	TIFFDirectory *td = &tif->tif_dir;
...
static void
JPEGDefaultTileSize(TIFF* tif, utiff_int32* tw, utiff_int32* th)
{
	JPEGState* sp = JState(tif);
	TIFFDirectory *td = &tif->tif_dir;
