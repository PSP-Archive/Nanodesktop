...
_TIFFPrintField(FILE* fd, const TIFFFieldInfo *fip,
		utiff_int32 value_count, void *raw_data)
{
	utiff_int32 j;
	...
	for(j = 0; j < value_count; j++) {
		if(fip->field_type == TIFF_BYTE)
			...
			fprintf(fd, "%u", ((utiff_int16 *) raw_data)[j]);
		else if(fip->field_type == TIFF_SSHORT)
			fprintf(fd, "%d", ((tiff_int16 *) raw_data)[j]);
		else if(fip->field_type == TIFF_LONG)
			fprintf(fd, "%lu",
				(unsigned long)((utiff_int32 *) raw_data)[j]);
		else if(fip->field_type == TIFF_SLONG)
			fprintf(fd, "%ld", (long)((tiff_int32 *) raw_data)[j]);
		else if(fip->field_type == TIFF_RATIONAL
			|| fip->field_type == TIFF_SRATIONAL
			|| fip->field_type == TIFF_FLOAT)
			fprintf(fd, "%f", ((float *) raw_data)[j]);
		else if(fip->field_type == TIFF_IFD)
			fprintf(fd, "0x%ulx", ((utiff_int32 *) raw_data)[j]);
		      ...
		      utiff_int32 value_count, void *raw_data)
{
	...
	switch (tag)
	{
		case TIFFTAG_INKSET:
			fprintf(fd, "  Ink Set: ");
			switch (*((utiff_int16*)raw_data)) {
				case INKSET_CMYK:
					fprintf(fd, "CMYK\n");
					break;
				default:
					fprintf(fd, "%u (0x%x)\n",
						*((utiff_int16*)raw_data),
						*((utiff_int16*)raw_data));
					break;
			}
			return 1;
		case TIFFTAG_DOTRANGE:
			fprintf(fd, "  Dot Range: %u-%u\n",
				((utiff_int16*)raw_data)[0], ((utiff_int16*)raw_data)[1]);
			return 1;
...
TIFFPrintDirectory(TIFF* tif, FILE* fd, long flags)
{
	...
	if (TIFFFieldSet(tif,FIELD_ROWSPERSTRIP)) {
		...
		if (td->td_rowsperstrip == (utiff_int32) -1)
			fprintf(fd, "(infinite)\n");
		else
