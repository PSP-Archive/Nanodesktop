 ...
 */
static int
TIFFSeek(TIFF* tif, utiff_int32 row, tsample_t sample)
{
...
int
TIFFReadScanline(TIFF* tif, tdata_t buf, utiff_int32 row, tsample_t sample)
{
	int e;
...
_TIFFSwab16BitData(TIFF* tif, tidata_t buf, tsize_t cc)
{
    (void) tif;
    assert((cc & 1) == 0);
    TIFFSwabArrayOfShort((utiff_int16*) buf, cc/2);
}
...
_TIFFSwab32BitData(TIFF* tif, tidata_t buf, tsize_t cc)
{
    (void) tif;
    assert((cc & 3) == 0);
    TIFFSwabArrayOfLong((utiff_int32*) buf, cc/4);
}
