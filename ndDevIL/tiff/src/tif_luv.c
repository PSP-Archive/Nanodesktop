...
LogLuvDecode24(TIFF* tif, tidata_t op, tsize_t occ, tsample_t s)
{
	...
	if (sp->user_datafmt == SGILOGDATAFMT_RAW)
		...
		assert(sp->tbuflen >= npixels);
		tp = (utiff_int32 *) sp->tbuf;
	}
					/* copy to array of utiff_int32 */
	bp = (unsigned char*) tif->tif_rawcp;
	cc = tif->tif_rawcc;
	...
	for (shft = 2*8; (shft -= 8) >= 0; )
		for (i = 0; i < npixels; i += rc) {
			...
			if (beg-i > 1 && beg-i < MINRUN) {
				b = (tiff_int16) (tp[i] & mask);/*check short run */
				j = i+1;
				while ((tp[j++] & mask) == b)
                                    if (j == beg) {
...
{
	tiff_int16* l16 = (tiff_int16*) sp->tbuf;
	float* yp = (float*) op;
...
L16fromY(LogLuvState* sp, tidata_t op, int n)
{
	...
	while (n-- > 0)
		*l16++ = (tiff_int16) (LogL16fromY(*yp++, sp->encode_meth));
}
...
#endif
void
LogLuv24toXYZ(utiff_int32 p, float XYZ[3])
{
	int	Ce;
	double	L, u, v, s, x, y;
					/* decode luminance */
...
{
	utiff_int32* luv = (utiff_int32*) sp->tbuf;
	float* xyz = (float*) op;
...
{
	utiff_int32* luv = (utiff_int32*) sp->tbuf;
	tiff_int16* luv3 = (tiff_int16*) op;

	while (n-- > 0) {
		double u, v;

		*luv3++ = (tiff_int16)((*luv >> 12 & 0xffd) + 13314);
		if (uv_decode(&u, &v, *luv&0x3fff) < 0) {
			u = U_NEU;
			v = V_NEU;
		}
		*luv3++ = (tiff_int16)(u * (1L<<15));
		*luv3++ = (tiff_int16)(v * (1L<<15));
		luv++;
	}
}
...
#endif
void
LogLuv32toXYZ(utiff_int32 p, float XYZ[3])
{
	double	L, u, v, s, x, y;
					/* decode luminance */
...
Luv32fromLuv48(LogLuvState* sp, tidata_t op, int n)
{
	...
	if (sp->encode_meth == SGILOGENCODE_NODITHER) {
		while (n-- > 0) {
			*luv++ = (utiff_int32)luv3[0] << 16 |
				(luv3[1]*(utiff_int32)(UVSCALE+.5) >> 7 & 0xff00) |
				(luv3[2]*(utiff_int32)(UVSCALE+.5) >> 15 & 0xff);
			luv3 += 3;
		}
		return;
	}
...
LogL16InitState(TIFF* tif)
{
	...
	case SGILOGDATAFMT_16BIT:
		sp->pixel_size = sizeof (tiff_int16);
		break;
	...
	if (multiply(sp->tbuflen, sizeof (tiff_int16)) == 0 ||
	    (sp->tbuf = (tidata_t*) _TIFFmalloc(sp->tbuflen * sizeof (tiff_int16))) == NULL) {
...
LogLuvInitState(TIFF* tif)
{
	...
	case SGILOGDATAFMT_FLOAT:
		...
		break;
	case SGILOGDATAFMT_16BIT:
		sp->pixel_size = 3*sizeof (tiff_int16);
		break;
	case SGILOGDATAFMT_RAW:
		sp->pixel_size = sizeof (utiff_int32);
		break;
	case SGILOGDATAFMT_8BIT:
	...
	sp->tbuflen = multiply(td->td_imagewidth, td->td_rowsperstrip);
	if (multiply(sp->tbuflen, sizeof (utiff_int32)) == 0 ||
	    (sp->tbuf = (tidata_t*) _TIFFmalloc(sp->tbuflen * sizeof (utiff_int32))) == NULL) {
		TIFFErrorExt(tif->tif_clientdata, module, "%s: No space for SGILog translation buffer",
		    tif->tif_name);
		return (0);
	}
