...
int
TIFFWriteScanline(TIFF* tif, tdata_t buf, utiff_int32 row, tsample_t sample)
{
...
TIFFWriteTile(TIFF* tif,
    tdata_t buf, utiff_int32 x, utiff_int32 y, utiff_int32 z, tsample_t s)
{
...
TIFFSetupStrips(TIFF* tif)
{
	...
	td->td_stripoffset = (utiff_int32 *)
	    _TIFFmalloc(td->td_nstrips * sizeof (utiff_int32));
	 ...
	 */
	_TIFFmemset(td->td_stripoffset, 0, td->td_nstrips*sizeof (utiff_int32));
	_TIFFmemset(td->td_stripbytecount, 0, td->td_nstrips*sizeof (utiff_int32));
...
{
	TIFFDirectory	*td = &tif->tif_dir;
	utiff_int32		*new_stripoffset, *new_stripbytecount;

	assert(td->td_planarconfig == PLANARCONFIG_CONTIG);
	new_stripoffset = (utiff_int32*)_TIFFrealloc(td->td_stripoffset,
		(td->td_nstrips + delta) * sizeof (utiff_int32));
	new_stripbytecount = (utiff_int32*)_TIFFrealloc(td->td_stripbytecount,
		(td->td_nstrips + delta) * sizeof (utiff_int32));
