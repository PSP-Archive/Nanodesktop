...
FILE *gocroutputfile;          // Patched for nd
FILE *try_gocroutputfile;      // Patched for nd
...
static void process_arguments(job_t *job, int argn, char *argv[])
{
  ...
  for (i = 1; i < argn; i++) {
	...
	else                // All this section is patched for nd 
    {
         try_gocroutputfile = fopen (s1, "w");
         
         if (try_gocroutputfile)
         {
            gocroutputfile = try_gocroutputfile;                   
         }
         else
         {
...
void print_output(job_t *job) {
  ...
  while (line) {
    ...
    fputs(line, gocroutputfile);                                           // Patched for nd
    if (job->cfg.out_format==HTML) fputs("<br />", gocroutputfile);        // Patched for nd
    if (job->cfg.out_format!=XML)  fputc('\n', gocroutputfile);            // Patched for nd
    line = getTextLine(linecounter++);
  }
  free_textlines();
  
  if (gocroutputfile!=stdout) fclose (gocroutputfile);    // Patched for nd
}
...
int gocr_main(int argn, char *argv[]) 
{
  ...
  gocroutputfile = stdout;            // Patched for nd
