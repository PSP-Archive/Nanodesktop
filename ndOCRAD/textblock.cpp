...
Textblock::Textblock( const Rectangle & r,
                      std::vector< std::vector< Blob * > > & blobp_matrix ) throw()
  : Rectangle( r )
  {
  int cuts = blobp_matrix.size();
  std::vector< int > mean_height( cuts, 0 );
  std::vector< std::vector< Blob * > > pending( cuts );
  std::vector< std::vector< Blob * > > pending_tall( cuts );
  std::vector< std::vector< Blob * > > pending_short( cuts );

  // Classify blobs by height.
  for( int cut = 0; cut < cuts; ++cut )
    {
    const std::vector< Blob * > & blobp_vector = blobp_matrix[cut];
    if( !blobp_vector.size() ) continue;
    unsigned int samples = 0;
    std::vector< int > height_distrib;
    for( unsigned int i = 0; i < blobp_vector.size(); ++i )
      {
      unsigned int h = blobp_vector[i]->height();
      unsigned int w = blobp_vector[i]->width();
  ...
  for( int current_line = 0, cut = 0; cut < cuts; ++cut )
    ...
    for( unsigned int i = 0; i < pending_tall[cut].size(); ++i )
      ...
      if( b.height() <= 3 * mh &&
        ...
        {
        if( l.character( std::min( bi+1, l.characters() - 1 ) ).left() > b.hcenter() )
          l.shift_characterp( new Character( &b ), true );
        }
      }
    }
