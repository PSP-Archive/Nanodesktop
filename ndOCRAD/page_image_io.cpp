...
int otsu_th( const std::vector< std::vector< unsigned char > > & data,
  ...
  std::vector< float > cmom;	// cumulative moment. Patched for Nanodesktop: use float instead of long long
  ...
  const float cmom_max = cmom[maxval];
  ...
  for( int i = 0; i < maxval; ++i )
    if( chist[i] > 0 && chist[i] < re.size() )
      {
      float bvar = (float)cmom[i] / chist[i];
...
bool Page_image::crop( const Rational ltrb[4], const Rational & th ) throw()
  ...
  if( a > re.left() ) { if( a < re.right() ) re.left( a ); else return false; }

  if( ltrb[1] <= 1 ) a = top() + ( ltrb[1] * ( height() - 1 ) ).trunc();
  else a = ltrb[1].round();
  if( a > re.top() ) { if( a < re.bottom() ) re.top( a ); else return false; }

  if( ltrb[2] <= 1 ) a = left() + ( ltrb[2] * ( width() - 1 ) ).trunc();
  else a = ltrb[2].round();
  if( a < re.right() ) { if( a > re.left() ) re.right( a ); else return false; }

  if( ltrb[3] <= 1 ) a = top() + ( ltrb[3] * ( height() - 1 ) ).trunc();
  else a = ltrb[3].round();
  if( a < re.bottom() ) { if( a > re.top() ) re.bottom( a ); else return false; }
