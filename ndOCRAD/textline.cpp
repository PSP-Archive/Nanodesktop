...
Textline::Textline( const Textline & tl ) throw()
  : Track( tl ), _big_initials( tl._big_initials )
  {
...
Textline & Textline::operator=( const Textline & tl ) throw()
  ...
  if( this != &tl )
    {
    Track::operator=( tl );
    _big_initials = tl._big_initials;
...
void Textline::set_track() throw()
  ...
  for( unsigned int i = _big_initials; i < cpv.size(); ++i )
    if( !cpv[i]->maybe(' ') ) rv.push_back( *cpv[i] );
  Track::set_track( rv );
  }


void Textline::verify_big_initials() throw()
  {
  if( _big_initials )
    {
    const int mh = mean_height();
    while( _big_initials && cpv[_big_initials-1]->height() <= 2 * mh )
      --_big_initials;
    }
  }


Character & Textline::character( const int i ) const throw()
  {
  if( i < 0 || i >= characters() )
...
void Textline::delete_character( const int i ) throw()
  {
  if( i < 0 || i >= characters() )
    Ocrad::internal_error( "delete_character, index out of bounds" );
  if( i < _big_initials ) --_big_initials;
  delete cpv[i]; cpv.erase( cpv.begin() + i );
  }


int Textline::shift_characterp( Character * p, const bool big ) throw()
  {
  int i = characters();
  ...
  if( i < _big_initials ) ++_big_initials;
  else if( big ) _big_initials = i + 1;
  return i;
  }


bool Textline::insert_space( const int i, const bool tab ) throw()
  {
  if( i <= 0 || i >= characters() )
...
void Textline::join( Textline & tl ) throw()
  ...
  for( int i = 0; i < tl.characters(); ++i )
    shift_characterp( tl.cpv[i], i < tl._big_initials );
  tl._big_initials = 0; tl.cpv.clear();
  }
...
int Textline::mean_height() const throw()
  ...
  for( int i = _big_initials; i < characters(); ++i )
...
void Textline::dprint( const Control & control, bool graph, bool recursive )
  ...
  for( int i = 0; i < characters(); ++i )
    ...
    if( i < _big_initials ) c.dprint( control, c, graph, recursive );
