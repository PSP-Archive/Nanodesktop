...
int Character::area() const throw()
  {
  int a = 0;
  for( int i = 0; i < blobs(); ++i ) a += bpv[i]->area();
  return a;
  }


Blob & Character::blob( int i ) throw()
  {
  if( i < 0 || i >= blobs() )
...
Blob & Character::main_blob() throw()
  {
  int imax = 0;
...
void Character::shift_blobp( Blob * p ) throw()
  {
  add_rectangle( *p );
  int i = blobs() - 1;
  for( ; i >= 0; --i )
...
void Character::join( Character & c ) throw()
  {
  for( int i = 0; i < c.blobs(); ++i ) shift_blobp( c.bpv[i] );
  c.bpv.clear();
  }
