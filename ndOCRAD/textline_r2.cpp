...
/*      if( b.holes() == 1 && 25 * b.hole( 0 ).size() < b.size() &&
  ...
  for( int i = big_initials(); i < characters(); )		// FIXME this block sucks
    {
    if( !cpv[i]->guesses() || cpv[i]->guess( 0 ).code != 0 ) { ++i; continue; }
    Character c( *cpv[i] );
    delete_character( i );
    if( c.guesses() >= 3 && c.blobs() >= 1 )
      {
      int left = c.guess( 0 ).value;
      for( int g = 1; g < c.guesses(); ++g )
        {
        Blob & b = c.blob( c.blobs() - 1 );
        Rectangle re( left, b.top(), c.guess( g ).value, b.bottom() );
        b.add_rectangle( re );
        Blob b1( b, re ); b1.adjust_height(); b1.find_holes();
        Character c1( new Blob( b1 ) );
        for( int k = 0; k < c.blobs() - 1; ++k )
          if( !c.blob( k ).includes( re ) && re.includes_hcenter( c.blob( k ) ) )
  ...
  for( int i = big_initials(); i < characters() - 3; ++i )
    {
    Character & c1 = character( i );
