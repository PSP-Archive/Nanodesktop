...
inline int gcd( int n, int m ) throw()		// Greatest Common Divisor
{
  register int _n = n;
  register int _m = m;
  ...
  while( true )
    {
    if( _m ) _n %= _m; else return _n;
    if( _n ) _m %= _n; else return _m;
    }
}


inline int lcm( int n, int m ) throw()		// Least Common Multiple
{
  register int _n = n;
  register int _m = m;
  ...
  _n /= gcd( _n, _m ); _n *= _m;	// lcm( n, m ) == ( n * m ) / gcd( n, m )
  if( _n < 0 ) _n = -_n;
  return _n;
}
