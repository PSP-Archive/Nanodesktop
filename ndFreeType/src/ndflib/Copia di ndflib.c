...
{   
    ...
    if (NewTTFSize!=0)
    {
       ...
       {
            FT_Set_Char_Size ( FontHandlesArray [FntHandle].TTF_Face, NewTTFSize * 64, 0, 100, 0 );
...
int ndFLIB_StrLength (char *str, int FntHandle, int NewTTFSize)
{
    FT_GlyphSlot slot;
	FT_Matrix matrix;
	FT_Vector pen;
    ...
    int OldTTFSize;
    int MaximumWidth, LastLeftPixel;
	...
	if (SizeChanged)
	{
        FT_Set_Char_Size ( FontHandlesArray [FntHandle].TTF_Face, OldTTFSize * 64, 0, 100, 0 );                 
    }
	
	return LastLeftPixel;
}
