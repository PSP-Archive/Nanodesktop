...
extern void	ndfree ();
extern char *	ndmalloc ();
extern char *	ndrealloc ();
extern char *	strchr ();
extern int	ndstrlen ();
...
field_t * fieldread (file, delims, flags, maxf)
    ...
    linebuf = (char *) ndmalloc (field_line_inc);
    if (linebuf == NULL)
	return NULL;
     ...
     * Read in the line.
     */
    while (fgets0D (&linebuf[linesize], linemax - linesize, file)
      != NULL)
	{
	linesize += ndstrlen (&linebuf[linesize]);
	if (linebuf[linesize - 1] == '\n')
	    break;
	else
	    {
	    linemax += field_line_inc;
	    linebuf = (char *) ndrealloc (linebuf, linemax);
	    if (linebuf == NULL)
		return NULL;
	    }
	}
...
field_t * fieldmake (line, allocated, delims, flags, maxf)
    ...
    fieldp = (field_t *) ndmalloc (sizeof (field_t));
    if (fieldp == NULL)
	return NULL;
    fieldp->nfields = 0;
	...
	{
	line = fieldp->linebuf =
	  (char *) ndrealloc (fieldp->linebuf, linesize + 1);
...
static field_t * fieldparse (fieldp, line, delims, flags, maxf)
    ...
    fieldp->fields = (char **) ndmalloc (fieldmax * sizeof (char *));
	...
	*lineout = '\0';
	if (fieldp->nfields >= fieldmax)
	    ...
	    fieldp->fields =
	      (char **) ndrealloc (fieldp->fields, fieldmax * sizeof (char *));
	    if (fieldp->fields == NULL)
		{
		fieldfree (fieldp);
		return NULL;
	...
	{
	fieldp->fields = (char **) ndrealloc (fieldp->fields,
...
void fieldfree (fieldp)
	...
	return;
    if (fieldp->linebuf != NULL)
	ndfree ((char *) fieldp->linebuf);
    if (fieldp->fields != NULL)
	ndfree ((char *) fieldp->fields);
    ndfree ((char *) fieldp);
    }
